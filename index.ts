...
try {
  ...
  const prNumber = testing ? payload.pull_request.number : payload.issue.number;
  ...
  if (
    !testing &&
    commentCommands.split(',').indexOf(payload.comment.body) !== -1
  ) {
  ...
  await exec('gh', ['pr', 'checkout', prNumber.toString()], silentOption);
